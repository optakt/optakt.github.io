...
This set consists of multiple transactions that are invalid towards each other or it is a singleton set - then it contains a single transaction (in the case of a cirtuous transaction).
...
Conflicting transation have the equivalence relation because they are equivocations spending the same UTXO.

The parent connection to T' from T is expressed as `T' <— T`.
The relation `T' *<— T` is its reflexive [transitive closure](../../../glossary.md#Transitive-Closuse-of-a-Graph) meaning that there is path from T to T'.
If T' <— T, then every node in the system that has T will also have T' and will know about the same relation `T' <— T`.
If such a relation between these transactions do not exist, then no node will end up with T' <— T.
...
Each node can compute a confidence value `d U (T)` (confidence value for node `u` about transaction `T`) from the progeny as the sum of transaction T decendants cihts from the progeny and its own chit.
Each transaction initially has a chit of 0 before the node gets the query results.
If the node collects a threshold of α 'yes' votes from the query the value of transaction T chit is set to 1, otherwise it will stay foerever 0.
...
When a node `u` discovers a trancation `T` through a query, it starts a one-time query process by sampling k random peers and sending a message to them, after T is delivered to the node by onReceiveTx.
Node `u` answers a query by checking whether each T' such that `T' *<— T` is currently prefered among competing transactions `∀T'' ∈ PT'` from conflict set P of T'.
If every single ancestor T' fulfills this criterion, the transation is said to br strongly preferred and receives a yes-vote (1).
...
This process will yield a labeling of the DAG with a chit value and associated confidence for each transaction T.
...
Conflict set P for T5, T6 and T9 has 3 elements {T6, T7, T9}
...
When a transaction has a larger confidence its decendants are likely to collect chits in the future compared to others with lower confidence.
For example T2 has larger confidence than T3 and its decendants are more likely to collect chits in the future.
...
For virtuous transactions, T is accepted when it is the only transaction in its confliict set and has a confidence no less than a threshold of β1.
...
How Avalance entangles transactions:
...
If no such transaction exists, the loop will stall until a new transation is added to the Set T.
The node then select k number of peers and queries them.
If more than α of these peers return a positive response, the chit value is set to 1.
After this tep the loop updates the prefered transaction of each conflict set of the transactions in its ancestry.
Next, T is added to the set Q of queried transactions so it will never be queried again by the node.
...
Firt step is to add transaction T to `Set T` unless it is already included.
Atfer that step `u` determines in T is currently strongly preferred, and if it is `u` returns a positive response to peer `j`, otherwise returns a negative response.
In the example pseudocode it is assumed that if a node knows about T, then it recursively knows about T ancestry.
